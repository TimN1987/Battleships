...
public abstract class Game : IGame
{
    ...
    #region Events
    public event Func<object?, AttackStatusReport, Task>? ComputerOpeningMoveCompleted;
    #endregion //Events
    ...
    /// </summary>
    public async Task PlayComputerOpeningMove()
    {
        var turnReports = RunComputerMove(out bool gameOver);
        var attackReport = new AttackStatusReport(gameOver, turnReports);

        if (ComputerOpeningMoveCompleted != null)
        {
            foreach (var handler in ComputerOpeningMoveCompleted.GetInvocationList()
                         .Cast<Func<object?, AttackStatusReport, Task>>())
            {
                await handler(this, attackReport);
            }
        }
    }
    ...
    internal static SingleTurnReport GenerateTurnReport(int gridPosition, ShotType shotType, Board board)
    {
